  ...
  if (result) {
    ...
    return (
      <div className="animate-slide-up text-center py-3 px-4 border bg-void-800/80 relative overflow-hidden">
        {!isDraw && (
          <div
            className={cn("absolute inset-0 opacity-10", isP1Win ? "bg-neon-pink" : "bg-neon-cyan")}
          />
        )}

        <div className="relative">
          <div className="text-xs font-display tracking-[0.4em] text-white/80 uppercase mb-1">
  ...
  return (
    <div
      className={cn(
        ...
        isX ? "border-neon-pink/70" : "border-neon-cyan/70",
      )}
    >
      <div className="text-xs font-display tracking-[0.4em] text-white/80 uppercase mb-1">
        Current Turn
