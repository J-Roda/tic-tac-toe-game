...
function StatBox({ label, value, color }) {
  ...
  return (
    <div className={cn(
      ...
      <span className={cn('font-display font-black text-2xl sm:text-3xl tabular-nums', textColors[color])}>
        {value}
      </span>
      <span className="text-[10px] font-display tracking-widest text-white/40 uppercase truncate w-full text-center">
        {label}
      </span>
    </div>
  )
  ...
  return (
    <div className="w-full space-y-2">
      ...
      <div className="flex justify-between items-center pt-1 px-1">
        ...
        <span className="text-[10px] font-display neon-text-green">
          {roundCount}
        </span>
      </div>
    </div>
